...
def sample_formulation(seq,kmers):
    ...
    kmer_in_seq = []
    for i in range(len(seq)):#seq中kmer窗口个数
        kmer = seq[i:i+1]#seq分割为kmer序列
...
base = 'ATCG'
kmers = list(product(base, repeat=1))#kmer是长度为4bases的所有可能
...
feature_vectors.to_csv("class"+str(1)+"_"+'mer'+'.txt', index=False, header = False,encoding='gbk',float_format='%.4f',sep = " ")
