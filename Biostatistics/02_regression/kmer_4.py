...
def sample_formulation(seq,kmers):
    ...
    kmer_in_seq = []
    for i in range(len(seq)-3):#seq中kmer窗口个数
        kmer = seq[i:i+4]#seq分割为kmer序列
...
base = 'ATCG'
kmers = list(product(base, repeat=4))#kmer是长度为4bases的所有可能
filename ="/beegfs/home/zmz/homework/01_data/regression/x-2k_sequence.fa"#输入fasta文件
...
feature_vectors.to_csv("class"+str(4)+"_"+'mer'+'.txt', index=False, header = False,encoding='gbk',float_format='%.4f',sep = " ")
