...
for (i in 1:nrow(haberman_clear_data)) {
  if (haberman_clear_data[i,7]=='aliveness'){
...
ggplot(haberman_clear_data,aes(age))+
...
ggplot(haberman_clear_data,aes(age))+
  ...
  geom_density()+
  theme_set(theme_bw())
boxplot(age~surve_status, data = haberman_clear_data)
ggplot(haberman_clear_data,aes(x=surve_status,y=age,fill=surve_status))+
  geom_boxplot()
#正态分布检验
...
ggplot(haberman_clear_data,aes(year))+
...
ggplot(haberman_clear_data,aes(year))+
  ...
  geom_density()+
  theme_set(theme_bw())
ggplot(haberman_clear_data,aes(x=surve_status,y=year,fill=surve_status))+
  geom_boxplot()
#正态分布检验
...
ggplot(haberman_clear_data,aes(nodes))+
  geom_histogram(aes(y=..density..),stat='bin',color='black',fill='lightblue')+
  geom_density()+
  theme_set(theme_bw())+
  theme(panel.grid=element_blank())
ggplot(haberman_clear_data,aes(nodes))+
  ...
  geom_density()+
  theme_set(theme_bw())
ggplot(haberman_clear_data,aes(x=surve_status,y=nodes,fill=surve_status))+
  geom_boxplot()
#正态分布检验
...
ggcoxzph(z1)
ggforest(model2,data = haberman_clear_data)
```
...
#观察数据分布情况
ggplot(data = haberman_clear_data,aes(x=age,y=nodes,shape=surve_status,color=surve_status))+
  geom_point()
ggplot(data = haberman_clear_data,aes(x=age,y=year,shape=surve_status,color=surve_status))+
  geom_point()
ggplot(data = haberman_clear_data,aes(x=year,y=nodes,shape=surve_status,color=surve_status))+
  geom_point()
colors <- c("red", "blue")
...
```R
performance<-function(table,lt=list(),n=2){
  ...
  result<-paste('Sensitivity = ',round(sensitivity,n),
                ...
                '\nNegtive Predictive Value = ',round(NPP,n),
                '\nAccuracy = ',round(hitrate,n),'\n',sep = '')
  cat(result)
  return(list(sensitivity,specificity,PPP,NPP,hitrate))
}
glm.per<-performance(t.step.glm)
lda.per<-performance(t.lda)
...
lda.per<-t(as.data.frame(lda.per))
...
model_per<-rbind(glm.per,lda.per)
model_per<-rbind(model_per,qda.per)
model_per<-rbind(model_per,tree.per)
model_per<-rbind(model_per,foreast.per)
model_per<-rbind(model_per,svm.per)

indicator<-t(data.frame('sensitivity','specificity','PPP','NPP','hitrate',
   'sensitivity','specificity','PPP','NPP','hitrate',
   'sensitivity','specificity','PPP','NPP','hitrate',
   'sensitivity','specificity','PPP','NPP','hitrate',
   'sensitivity','specificity','PPP','NPP','hitrate',
   'sensitivity','specificity','PPP','NPP','hitrate'))
...
ggplot(model_per, aes(x=indicator, y=result, fill =model)) +
  ...
  scale_fill_manual(values = c("orange", "skyblue", "red",'green','brown','steelblue'))
  theme_bw() 

test_data$lda<-as.numeric(as.character(test_data$lda))
test_data$qda<-as.numeric(as.character(test_data$qda))
test_data$tree<-as.numeric(as.character(test_data$tree))
##ROC曲线的绘制
ROC_logistics<-roc(test_data$status,test_data$logistics)
ROC_lda<-roc(test_data$status,test_data$lda)
...
round(ci(auc(ROC_lda)),3)
round(ci(auc(ROC_qda)),3)
round(ci(auc(ROC_tree)),3)
round(ci(auc(ROC_foreast)),3)
round(ci(auc(ROC_svm)),3)
plot(1-ROC_logistics$specificities,ROC_logistics$sensitivities,type = 'l',col='red',lty=1,xlab = 'FP',ylab = 'TP',lwd=2)
lines(1-ROC_lda$specificities,ROC_lda$sensitivities,type = 'l',col='green',lty=1,lwd=2)
lines(1-ROC_qda$specificities,ROC_qda$sensitivities,type = 'l',col='blue',lty=1,lwd=2)
lines(1-ROC_tree$specificities,ROC_tree$sensitivities,type = 'l',col='brown',lty=1,lwd=2)
lines(1-ROC_foreast$specificities,ROC_foreast$sensitivities,type = 'l',col='orange',lty=1,lwd=2)
lines(1-ROC_svm$specificities,ROC_svm$sensitivities,type = 'l',col='pink',lty=1,lwd=2)
abline(0,1)
legend(0.6,0.35,
       c('logistics 0.564 [0.501,0.626]','lda 0.564 [0.501,0.626]','qda 0.587 [0.517 0.656]','tree  0.605 [0.529 0.681]','foreast 0.623 [0.543 0.704]','svm 0.497 [0.564 0.632]'),
       lty = c(2,2,2,2,2,2),
       lwd = c(5,5,5,5,5,5),
       col = c('red','green','blue','brown','orange','pink'),
       bty = 'n'
)
legend(0.75,0.38,'AUC',bty='n')
```
